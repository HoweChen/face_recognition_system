...
class Camera:
    ...
    def serve_single_image(self, input_img_str: str) -> str:
        # this method will accept an base64 encoded string as input, and return the username of that user
        if input_img_str[0:23] == "data:image/jpeg;base64,":
            input_img_str = input_img_str[23::]  # remove the prefix of input image string
        # since it's an single image serving method, then there is no need for process_this_frame for performance improvement
        frame = string_to_image(input_img_str)
        small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)
        rgb_small_frame = small_frame[:, :, ::-1]
        self.face_detection(rgb_small_frame)
        self.face_matching()
        return self.face_names[0]  # we print the first name of the name list
...
#
#
# def face_detection(input_frame, mode="MTCNN"):
#     face_locations = []
#     face_encodings = []
#     if mode == "MTCNN":
#         detector = MTCNN()
#         try:
#             detect_result = [face["box"] for face in detector.detect_faces(input_frame)]
#         except IndexError:
#             # which means the detector doesn't find the faces, then directly show the image without box
#             detect_result = None
#         if detect_result is not None:
...
#
#
# def face_matching(face_locations, face_encodings, redis_connection):
#     face_names = []
#     best_point_list = []
#
...
#         # See if the face is a match for the known face(s)
#         true_or_false, points = face_recognition.compare_faces(
#                 list(map(lambda x: np.frombuffer(x), redis_connection.lrange("known_face_encodings", 0, -1))),
#                 face_encoding,
#                 tolerance=0.4)
#         name = "Unknown"
#         best_point = None
...
#             winner_index = points.index(best_point)
#             print(winner_index)
#             name = redis_connection.lrange("known_face_names", 0, -1)[winner_index].decode("utf-8")
#         face_names.append(name)
#         best_point_list.append(best_point)
#     return face_names, best_point_list
#
#
# def render_box(frame, face_locations, face_names, best_point_list):
#     # Display the results
...
#     frame = string_to_image(input_image_str)
#     small_frame = cv2.resize(frame, (0, 0), fx=0.25, fy=0.25)
#     rgb_small_frame = small_frame[:, :, ::-1]
#     face_locations, face_encodings = face_detection(rgb_small_frame, mode)
#     face_names, best_point_list = face_matching(face_locations, face_encodings, redis_connection)
#     tmp_output_image = render_box(frame, face_locations, face_names, best_point_list)
#     output_image_str = image_to_str(tmp_output_image)
#     # cv2.imshow("Video", tmp_output_image)
